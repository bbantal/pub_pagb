...
This script investigates age trends in network instability for the Camcan dataset.
...
SRCDIR = "/shared/datasets/public/camcan/derivatives/network_instability/"
...
# Load age info
demo = pd \
    .read_csv("/shared/datasets/public/camcan/sourcedata/meta/participant-info_camcan.csv") \
    .rename({"subj": "subject", "gender": "sex"}, axis=1)

# Network instability
data = pd.read_csv(SRCDIR + "brain_network_stability_20240424_182312.csv") \
...
sns.lineplot(data=df, x="age_group", y="stability", ci=68,
            ...
            err_kws={"capsize": 0, "capthick": 1.5, "linewidth": 1.5},
             markeredgecolor ="k",
             color="k", zorder=10)
plt.plot(xfit, yfit, color="black", lw=1, zorder=2)
...
# Format
plt.title(f"CamCAN Dataset (N={df.shape[0]:,})")
...
plt.savefig(OUTDIR + "fig_camcan_sigmoid_subnetwork.pdf", transparent=True, dpi=300)
...
res["xfit"] = xfit
res["yfit"] = yfit
with open(OUTDIR + "data_camcan_subnetwork.pkl", "wb") as f:
    pickle.dump(res, f)
